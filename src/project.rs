...
use crate::{
    models::{Config, General, Pack, PackMeta},
...
pub fn create_project(name: String, path: Option<PathBuf>) {
    let default_new_dir: PathBuf = match env::var("PRIS_TEST_FOLDER") {
        Ok(new_path) => PathBuf::from(new_path),
        Err(_) => PathBuf::new(),
    };
    ...
    match project_path.exists() {
        true => match project_path.read_dir() {
            Ok(mut dir_contents) => {
        ...
        false => match fs::create_dir_all(&project_path) {
            Ok(_) => (),
            Err(err) => {
                println!("{}", err);
                exit(1)
    ...
    match fs::write(project_path.join("prismarine.toml"), config_string) {
        ...
        Err(err) => {
            pris_err!(format!(
                "Couldn't write to file `prismarine.toml`: {}",
                err.to_string()
            ));
            exit(1)
        }
    ...
    match fs::write(
        project_path.join(".prisignore"),
        ".git\n.gitignore\n/source\n*.env\n*.zip\n",
    ) {
        Ok(_) => (),
        Err(err) => {
            pris_err!(format!(
                "Couldn't write to file `.prisignore`: {}",
                err.to_string()
            ));
            exit(1)
        }
    ...
    match fs::write(
        ...
        format!(
            "# {}\n\nThis is a resource pack made with Prismarine!",
            &name
        ),
    ) {
        Ok(_) => (),
        Err(err) => {
            pris_err!(format!(
                "Couldn't write to file `README.md`: {}",
                err.to_string()
            ));
            exit(1)
        }
    ...
    println!("\n✨ Project `{}` created! ✨\n", &name.italic().bold());
    println!(
        "Find out how to configure it at: {}",
        "https://prismarine.jadelily.dev".bright_blue()
    );
}
    ...
    */
    ...
    for entry in included_items {
        ...
        let mut name = match path.strip_prefix(".") {
            ...
            Err(_) => {
                pris_err!(format!(
                    "Couldn't strip prefix for item: {}",
                    path.to_str().unwrap()
                ));
                exit(1)
            }
        ...
        if name.starts_with("pack") {
            name = match name.strip_prefix("pack") {
                ...
                Err(_) => {
                    pris_err!(format!(
                        "Couldn't strip prefix `pack` from: {}",
                        &name.to_str().unwrap()
                    ));
                    exit(1)
                }
        ...
        if path.is_dir() {
            ...
            #[allow(deprecated)]
            // .add_directory_from_path() is deprecated, but it works well for now!
            match zip_writer.add_directory_from_path(name, zip_options) {
                Ok(_) => (),
                Err(_) => {
                    pris_err!(format!(
                        "Couldn't add directory to zip: {}",
                        &name.to_str().unwrap()
                    ));
                    exit(1)
                }
            ...
            match zip_writer.write(&file_buffer) {
                ...
                Err(_) => {
                    pris_err!(format!(
                        "Couldn't write to zip from file: {}",
                        &name.to_str().unwrap()
                    ));
                    exit(1)
                }
